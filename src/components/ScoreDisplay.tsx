        ...
        <div style={{ textAlign: 'left', marginTop: '2rem' }}>
          <h3 style={{ marginBottom: '1rem' }}>
            ...
            <span style={{
              fontSize: '0.9rem',
              marginLeft: '0.5rem',
              color: report.metadataScore.totalScore >= 20 ? 'var(--success-color)' :
                     report.metadataScore.totalScore >= 15 ? 'var(--primary-color)' :
                     report.metadataScore.totalScore >= 10 ? 'var(--warning-color)' :
                     'var(--danger-color)',
              fontWeight: 'normal'
            }}>
              ({report.metadataScore.totalScore} / 25 points)
            </span>
          </h3>
          ...
          <div style={{
            ...
            <div>
              <strong>Maturity:</strong> {report.metadataScore.breakdown.maturity} / 8
              {report.metadataScore.details.userCount && (
                <div style={{ fontSize: '0.85rem', color: '#888' }}>
                  {report.metadataScore.details.userCount.toLocaleString()} users
                </div>
              )}
            </div>
            <div>
              <strong>Transparency:</strong> {report.metadataScore.breakdown.transparency} / 7
              <div style={{ fontSize: '0.85rem', color: '#888' }}>
                {report.metadataScore.details.hasContact && '✓ Contact '}
                {report.metadataScore.details.hasPrivacyPolicy && '✓ Privacy '}
                {report.metadataScore.details.hasSecurityContact && '✓ Security'}
              </div>
            </div>
            <div>
              <strong>Registration:</strong> {report.metadataScore.breakdown.registration} / 5
              <div style={{ fontSize: '0.85rem', color: '#888' }}>
                {report.metadataScore.details.registrationPolicy}
              </div>
            </div>
            <div>
              <strong>Description:</strong> {report.metadataScore.breakdown.description} / 5
              <div style={{ fontSize: '0.85rem', color: '#888' }}>
                {report.metadataScore.details.descriptionQuality} ({report.metadataScore.details.descriptionLength} chars)
              </div>
            </div>
          </div>
          ...
          <details style={{ marginTop: '1rem' }}>
            ...
            <div style={{ paddingLeft: '1rem', marginTop: '0.5rem', fontSize: '0.9rem' }}>
              <div style={{ marginBottom: '0.5rem' }}>
                <strong>Administrative Transparency:</strong>
                <ul style={{ marginLeft: '1rem', marginTop: '0.25rem' }}>
                  <li style={{ color: report.metadataScore.details.hasContact ? 'var(--success-color)' : 'var(--danger-color)' }}>
                    {report.metadataScore.details.hasContact ? '✓' : '✗'} Contact Information
                  </li>
                  <li style={{ color: report.metadataScore.details.hasPrivacyPolicy ? 'var(--success-color)' : 'var(--danger-color)' }}>
                    {report.metadataScore.details.hasPrivacyPolicy ? '✓' : '✗'} Privacy Policy
                  </li>
                  <li style={{ color: report.metadataScore.details.hasTerms ? 'var(--success-color)' : 'var(--danger-color)' }}>
                    {report.metadataScore.details.hasTerms ? '✓' : '✗'} Terms of Service
                  </li>
                  <li style={{ color: report.metadataScore.details.hasSecurityContact ? 'var(--success-color)' : 'var(--danger-color)' }}>
                    {report.metadataScore.details.hasSecurityContact ? '✓' : '✗'} Security Contact (security.txt)
                  </li>
                </ul>
              </div>
              {report.metadataScore.details.ageDays && (
                <div style={{ marginBottom: '0.5rem' }}>
                  <strong>Estimated Age:</strong> ~{Math.floor(report.metadataScore.details.ageDays / 30)} months
                </div>
              )}
            </div>
          </details>
